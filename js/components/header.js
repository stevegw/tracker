...
const HeaderComponent = {
    ...
    initCloudSyncSettings() {
        ...
        if (config.clientId && config.apiKey) {
            const actionsDiv = document.getElementById('cloud-sync-actions');
            if (actionsDiv) actionsDiv.style.display = 'block';
        }

        // Save configuration button
        const saveConfigBtn = document.getElementById('save-cloud-config-btn');
        if (saveConfigBtn) {
            saveConfigBtn.addEventListener('click', async () => {
                const clientId = clientIdInput?.value.trim();
                const apiKey = apiKeyInput?.value.trim();
                const autoSync = autoSyncToggle?.checked !== false;

                if (!clientId || !apiKey) {
                    UIController.showToast('Please enter both Client ID and API Key', 'warning');
                    return;
                }

                CloudSync.saveConfig(clientId, apiKey, autoSync);
                UIController.showToast('Configuration saved', 'success');
        ...
        if (syncNowBtn) {
            syncNowBtn.addEventListener('click', async () => {
                ...
                UIController.showToast('Sync completed', 'success');
            });
        }
    }
};
